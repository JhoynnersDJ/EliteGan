...
import { calcularDiferenciaDeTiempo, calculartarifa, formatHour, esDiaActualOAnterior } from '../libs/Tarifa.js';
...
export const register = async (req, res) => {
    ...
    try {
        if (!esDiaActualOAnterior(fecha)) { return res.status(404).json({ message: 'Fecha Invalida' }); }
        ...
        if (time2.fin) {
            ...
            const tareaSaved_dia1 = new tarea(null, fecha, horas1.tiempo_formateado1, '00:00AM',
                ...
                id_proyecto, id_servicio, time2.tarifa1 * proyectFound.tarifa, status, null, id_usuario);
            ...
            const tareaSaved_dia2 = new tarea(null, time2.fin, '00:00AM', horas2.tiempo_formateado2,
                ...
                id_proyecto, id_servicio, time2.tarifa2 * proyectFound.tarifa, status, null, id_usuario);
            ...
            const tareaSaved = new tarea(null, fecha, horas.tiempo_formateado1, horas.tiempo_formateado2,
                time.tiempo_minutos, time2.tarifa1, id_proyecto, id_servicio, time2.tarifa1 * proyectFound.tarifa, status, null, id_usuario)
...
export const updateTask = async (req, res) => {
    ...
    try {
        ...
        if (id_servicio) {
            ...
            if (!serviceFound) return res.status(404).json({ message: 'Servicio no encontrado' })
        } else { id_servicio = taskFound.id_servicio }

        if (!hora_inicio) { hora_inicio = taskFound.hora_inicio }

        if (!hora_fin) { hora_fin = taskFound.hora_fin }

        if (!fecha) { fecha = taskFound.fecha }

        if (typeof status === 'undefined') { status = taskFound.status }
        ...
        const tareaSaved = new tarea(taskFound.id_tarea, fecha, horas.tiempo_formateado1, horas.tiempo_formateado2,
            time.tiempo_minutos, time2.tarifa1, taskFound.id_proyecto, id_servicio,
            time2.tarifa1 * proyectFound.tarifa, status, null, taskFound.id_usuario)
